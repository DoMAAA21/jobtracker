...
Web ignores `accessToken` and uses the cookie. Mobile stores `accessToken` and sends `Authorization: Bearer …`.
...
`login` and `register` return **`accessToken` in the JSON body** and still set the httpOnly cookie for web:
...
3. **Store token** — save `accessToken` from response (SecureStore / Keychain).  
...
4. **Mobile** = Bearer header; read `accessToken` from login/register JSON.  
